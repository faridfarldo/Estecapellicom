<?php
/**
 * Spanish importer for the eight Hair Transplant treatments.
 *
 * Imports one flexible-content treatment per request to remain below ACFML's
 * execution-time limit. English owns structure and media; the Spanish JSON
...
if ( ! defined( 'ESTECAPELLI_ES_HAIR_IMPORT_VERSION' ) ) {
	define( 'ESTECAPELLI_ES_HAIR_IMPORT_VERSION', '2026-07-17.1' );
}

/** English source slug => exact indexed Spanish slug. */
function estecapelli_es_hair_manifest() {
	return array(
		...
		'sapphire-fue-hair-transplant' => 'trasplante-capilar-fue-sapphire',
		'dhi-hair-transplant'          => 'trasplante-de-cabello-dhi',
...
/** Load and strictly validate every Spanish treatment overlay. */
function estecapelli_es_hair_load_translations() {
	$directory = get_template_directory() . '/inc/data/translations/es/hair-transplant';
	$indexed   = estecapelli_indexed_treatment_slugs();
	$loaded    = array();

	foreach ( estecapelli_es_hair_manifest() as $source_slug => $spanish_slug ) {
		if ( ( $indexed[ $source_slug ]['es'] ?? '' ) !== $spanish_slug ) {
			return new WP_Error( 'es_hair_indexed_slug_mismatch', sprintf( 'The Spanish slug does not match the indexed URL contract: %s.', $source_slug ) );
		}

		$file = $directory . '/' . $source_slug . '.json';
		if ( ! is_readable( $file ) ) {
			return new WP_Error( 'es_hair_missing_file', sprintf( 'Missing Spanish translation file: %s.', basename( $file ) ) );
		}
		$translation = json_decode( (string) file_get_contents( $file ), true );
		if ( ! is_array( $translation ) ) {
			return new WP_Error( 'es_hair_invalid_json', sprintf( 'Invalid JSON in %s: %s', basename( $file ), json_last_error_msg() ) );
		}
		if (
			...
			return new WP_Error( 'es_hair_invalid_translation', sprintf( 'Incomplete or mismatched Spanish translation: %s.', basename( $file ) ) );
		}
...
/** Import or repair one Spanish Hair Transplant treatment. */
function estecapelli_run_es_hair_import( $source_slug ) {
	$source_slug = (string) $source_slug;
	if ( ! isset( estecapelli_es_hair_manifest()[ $source_slug ] ) ) {
		return new WP_Error( 'es_hair_invalid_source', sprintf( 'Unknown Spanish Hair Transplant source: %s.', $source_slug ) );
	}
	if ( ! function_exists( 'update_field' ) ) {
		return new WP_Error( 'es_hair_acf_missing', 'ACF is required for the Spanish Hair Transplant import.' );
	}
	if ( ! defined( 'ICL_SITEPRESS_VERSION' ) && ! defined( 'WPML_VERSION' ) ) {
		return new WP_Error( 'es_hair_wpml_missing', 'WPML is required for the Spanish Hair Transplant import.' );
	}

	$languages = apply_filters( 'wpml_active_languages', null, array( 'skip_missing' => 0 ) );
	if ( ! is_array( $languages ) || ! isset( $languages['es'] ) ) {
		return new WP_Error( 'es_hair_spanish_inactive', 'Spanish must be active in WPML before importing treatment translations.' );
	}

	$translations = estecapelli_es_hair_load_translations();
	...
	$spanish_term_id = estecapelli_it_hair_category(
		array(
			...
			'target_slug'          => 'trasplante-capilar',
			'target_name'          => 'Trasplante capilar',
			'target_language'      => 'es',
...
add_action( 'admin_menu', 'estecapelli_register_es_hair_importer' );
/** Register the dedicated Spanish Hair Transplant importer. */
function estecapelli_register_es_hair_importer() {
	add_management_page(
		__( 'Spanish Hair Transplant Importer', 'estecapelli' ),
		__( 'Spanish Hair Transplant', 'estecapelli' ),
		'manage_options',
		'estecapelli-spanish-hair-importer',
		'estecapelli_render_es_hair_importer'
	);
}

add_action( 'admin_post_estecapelli_import_es_hair', 'estecapelli_handle_es_hair_import' );
/** Process one Spanish treatment and return to the importer. */
function estecapelli_handle_es_hair_import() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You are not allowed to import content.', 'estecapelli' ) );
	}
	check_admin_referer( 'estecapelli_import_es_hair' );

	$source_slug = isset( $_POST['source'] ) ? sanitize_title( wp_unslash( $_POST['source'] ) ) : '';
	$result      = estecapelli_run_es_hair_import( $source_slug );
	if ( is_wp_error( $result ) ) {
		set_transient( 'estecapelli_es_hair_import_error', $source_slug . ': ' . $result->get_error_message(), 5 * MINUTE_IN_SECONDS );
	} else {
		set_transient( 'estecapelli_es_hair_import_success', $source_slug, 5 * MINUTE_IN_SECONDS );
	}

	wp_safe_redirect( add_query_arg( 'page', 'estecapelli-spanish-hair-importer', admin_url( 'tools.php' ) ) );
	exit;
}

/** Render the one-treatment-per-request importer screen. */
function estecapelli_render_es_hair_importer() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	$success = get_transient( 'estecapelli_es_hair_import_success' );
	$error   = get_transient( 'estecapelli_es_hair_import_error' );
	delete_transient( 'estecapelli_es_hair_import_success' );
	delete_transient( 'estecapelli_es_hair_import_error' );
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Spanish Hair Transplant Importer', 'estecapelli' ); ?></h1>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<input type="hidden" name="action" value="estecapelli_import_es_hair">
			<?php wp_nonce_field( 'estecapelli_import_es_hair' ); ?>
			<table class="widefat striped" style="max-width:980px;margin-top:1rem;">
				...
				<tbody>
					<?php foreach ( estecapelli_es_hair_manifest() as $source_slug => $spanish_slug ) :
