<?php
/**
 * Portuguese importer for the eight Hair Transplant treatments.
 *
 * Imports one flexible-content treatment per request to remain below ACFML's
 * execution-time limit. English owns structure and media; the Portuguese JSON
...
if ( ! defined( 'ESTECAPELLI_PT_HAIR_IMPORT_VERSION' ) ) {
	define( 'ESTECAPELLI_PT_HAIR_IMPORT_VERSION', '2026-07-17.1' );
}

/** English source slug => exact indexed Portuguese slug. */
function estecapelli_pt_hair_manifest() {
	return array(
		...
		'sapphire-fue-hair-transplant' => 'transplante-capilar-fue-sapphire',
...
/** Load and strictly validate every Portuguese treatment overlay. */
function estecapelli_pt_hair_load_translations() {
	$directory = get_template_directory() . '/inc/data/translations/pt/hair-transplant';
	$indexed   = estecapelli_indexed_treatment_slugs();
	$loaded    = array();

	foreach ( estecapelli_pt_hair_manifest() as $source_slug => $portuguese_slug ) {
		if ( ( $indexed[ $source_slug ]['pt'] ?? '' ) !== $portuguese_slug ) {
			return new WP_Error( 'pt_hair_indexed_slug_mismatch', sprintf( 'The Portuguese slug does not match the indexed URL contract: %s.', $source_slug ) );
		}

		$file = $directory . '/' . $source_slug . '.json';
		if ( ! is_readable( $file ) ) {
			return new WP_Error( 'pt_hair_missing_file', sprintf( 'Missing Portuguese translation file: %s.', basename( $file ) ) );
		}
		$translation = json_decode( (string) file_get_contents( $file ), true );
		if ( ! is_array( $translation ) ) {
			return new WP_Error( 'pt_hair_invalid_json', sprintf( 'Invalid JSON in %s: %s', basename( $file ), json_last_error_msg() ) );
		}
		if (
			...
			return new WP_Error( 'pt_hair_invalid_translation', sprintf( 'Incomplete or mismatched Portuguese translation: %s.', basename( $file ) ) );
		}
...
/** Import or repair one Portuguese Hair Transplant treatment. */
function estecapelli_run_pt_hair_import( $source_slug ) {
	$source_slug = (string) $source_slug;
	if ( ! isset( estecapelli_pt_hair_manifest()[ $source_slug ] ) ) {
		return new WP_Error( 'pt_hair_invalid_source', sprintf( 'Unknown Portuguese Hair Transplant source: %s.', $source_slug ) );
	}
	if ( ! function_exists( 'update_field' ) ) {
		return new WP_Error( 'pt_hair_acf_missing', 'ACF is required for the Portuguese Hair Transplant import.' );
	}
	if ( ! defined( 'ICL_SITEPRESS_VERSION' ) && ! defined( 'WPML_VERSION' ) ) {
		return new WP_Error( 'pt_hair_wpml_missing', 'WPML is required for the Portuguese Hair Transplant import.' );
	}
	...
	$portuguese_term_id = estecapelli_it_hair_category(
		array(
			...
			'target_slug'          => 'transplante-capilar',
			'target_name'          => 'Transplante capilar',
...
add_action( 'admin_menu', 'estecapelli_register_pt_hair_importer' );
/** Register the dedicated Portuguese Hair Transplant importer. */
function estecapelli_register_pt_hair_importer() {
	add_management_page(
		__( 'Portuguese Hair Transplant Importer', 'estecapelli' ),
		__( 'Portuguese Hair Transplant', 'estecapelli' ),
		'manage_options',
		'estecapelli-portuguese-hair-importer',
		'estecapelli_render_pt_hair_importer'
	);
}

add_action( 'admin_post_estecapelli_import_pt_hair', 'estecapelli_handle_pt_hair_import' );
/** Process one Portuguese treatment and return to the importer. */
function estecapelli_handle_pt_hair_import() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You are not allowed to import content.', 'estecapelli' ) );
	}
	check_admin_referer( 'estecapelli_import_pt_hair' );

	$source_slug = isset( $_POST['source'] ) ? sanitize_title( wp_unslash( $_POST['source'] ) ) : '';
	$result      = estecapelli_run_pt_hair_import( $source_slug );
	if ( is_wp_error( $result ) ) {
		set_transient( 'estecapelli_pt_hair_import_error', $source_slug . ': ' . $result->get_error_message(), 5 * MINUTE_IN_SECONDS );
	} else {
		set_transient( 'estecapelli_pt_hair_import_success', $source_slug, 5 * MINUTE_IN_SECONDS );
	}

	wp_safe_redirect( add_query_arg( 'page', 'estecapelli-portuguese-hair-importer', admin_url( 'tools.php' ) ) );
	exit;
}

/** Render the one-treatment-per-request importer screen. */
function estecapelli_render_pt_hair_importer() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	$success       = get_transient( 'estecapelli_pt_hair_import_success' );
	$error         = get_transient( 'estecapelli_pt_hair_import_error' );
	$wpml_language = estecapelli_wpml_language_code( 'pt' );
	delete_transient( 'estecapelli_pt_hair_import_success' );
	delete_transient( 'estecapelli_pt_hair_import_error' );
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Portuguese Hair Transplant Importer', 'estecapelli' ); ?></h1>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<input type="hidden" name="action" value="estecapelli_import_pt_hair">
			<?php wp_nonce_field( 'estecapelli_import_pt_hair' ); ?>
			<table class="widefat striped" style="max-width:980px;margin-top:1rem;">
				...
				<tbody>
					<?php foreach ( estecapelli_pt_hair_manifest() as $source_slug => $portuguese_slug ) :
